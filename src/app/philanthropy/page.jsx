...
// Dummy data for sidebar (replace with real icons if needed)
const sections = [
...
export default function PhilanthropyPage() {
  ...
  return (
    <>
      ...
      <div className="max-w-screen mx-auto flex flex-col md:flex-row gap-8 px-4 md:px-8 py-10 bg-[#e3ffe0] text-gray-800">
        {/* Sidebar */}
        <aside className="hidden md:flex flex-col gap-8 w-64 h-fit sticky top-24">
          {/* Sidebar 1 */}
          <nav className=" space-y-2">
            ...
            {sections.map((sec) => (
              <Link
                key={sec.id}
                href={sec.id}
                className="flex items-center gap-2 px-2 py-1 rounded  hover:text-green-700 transition"
              >
                <span>{sec.icon}</span>
                {sec.label}
              </Link>
          ...
          <nav className="space-y-2">
            ...
            <Link href="/aboutus/aboutus" className="block px-3 py-2 rounded  hover:text-green-700 transition">
              About Us
            </Link>
            <Link href="/aboutus/fundamentalsofhomeopathy" className="block px-3 py-2 rounded  hover:text-green-900 transition">
              Fundamentals of Homeopathy
            </Link>
            <Link href="/aboutus/myths&factsabouthomeopathy" className="block px-3 py-2 rounded  hover:text-green-900 transition">
              Myths & Facts About Homeopathy
            </Link>
            <Link href="/aboutus/visionandmission" className="block px-3 py-2 rounded  hover:text-green-900 transition">
              Vision and Mission
            </Link>
            <Link href="/aboutus/teambalance" className="block px-3 py-2 rounded  hover:text-green-900 transition">
              Team Balance
            </Link>
          </nav>
        </aside>
          ...
          <section data-aos="zoom-in">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {["/phi1.jpeg", "/phi2.jpeg", "/phi3.jpeg", "/phi4.jpeg"].map((src, index) => (
                <div key={index} className="w-full h-40 relative rounded-xl overflow-hidden shadow hover:scale-105 transition-transform">
